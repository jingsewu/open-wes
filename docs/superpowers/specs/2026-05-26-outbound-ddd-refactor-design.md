...
| Phase 6 | **Sync coding standards**: Update `docs/standards/backend.md` and `CLAUDE.md` to reflect new patterns | Low - documentation only |
...
#### `docs/standards/backend.md` (Existing File — Update)
This file already exists with comprehensive DDD rules. Update the following sections:

- **Domain Entity** (Section): Add `@Getter @Builder` as the required annotation set. Explicitly forbid `@Data` on domain entities. Add `@NoArgsConstructor(access = AccessLevel.PROTECTED)` for JPA/MapStruct compatibility. Document static factory method as preferred construction pattern.
  ```java
  // Required annotations for domain entities:
  @Getter
  @Builder
  @AllArgsConstructor(access = AccessLevel.PRIVATE)
  ...
  @EqualsAndHashCode(callSuper = true)
  ```
- **Domain Aggregate** (Section): Rename/replace with **Use Case Pattern**. Document that `domain/aggregate/` is deprecated. Cross-aggregate orchestration belongs in `application/usecase/` classes. Add UseCase naming convention: `[Action][Entity]UseCase`.
- **Domain Service** (Section): Add explicit prohibition — no `@Transactional`, no external API calls, no persistence operations. Domain services are pure stateless calculation only.
- **Domain Events** (Section): Add event subscriber rules — must be thin dispatchers, delegate to UseCase, must have `@Transactional(rollbackFor = Exception.class)` on write paths. No business logic in subscribers.
- **Transaction** (Section): Update to reflect that `@Transactional` is now on **UseCases and Repositories**, not "Aggregates and Repositories". Document propagation behavior (REQUIRED by default, subscriber and UseCase share transaction).
- **API Implementation** (Section): Add that ApiImpl should be a thin adapter — delegate write operations to UseCases, only handle DTO conversion and read-only queries directly.
- **Class Naming Conventions**: Add UseCase row to the naming table: `[Action][Entity]UseCase` | `CancelOutboundPlanOrderUseCase`
